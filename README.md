...
<p align="center">
  <i>README was generated by AI because I was too lazy to explain. I just copied the KoroneStrap from another project, so I'm grateful to them. I only tried to fix a few things and avoid downloading dependencies, especially with Proton-GE, so I didn't pay much attention to that, so there must be errors. There will be better projects than this one, and I hope so, I won't be able to fix much because I prefer to use other languages ​​instead of Python. I only used it because KoroneStrap required it and I didn't want to rewrite it, so the code is "disgusting" to say the least, Version 2 will have support for Linux and mobile, so this will be useless, and when it's useless I'll delete this repository, but it worked perfectly on various Debian distributions</i>
</p>
